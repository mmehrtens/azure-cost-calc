...
$T_{backup}$ | hours | Time window for incremental backups
...
Throughput required for incremental backup in MB/s | $\Phi_{incr}$ | $=$ | $\big(V_{veeam}^{incr}\cdot 1024^2\big)\div\big(T_{backup}\cdot 3600\big)$
